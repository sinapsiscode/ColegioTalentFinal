...
const ActiveUsers = ({ usuarios, loading = false, onUserClick }) => {
  if (loading) {
    return (
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-2 sm:gap-0">
        ...
        <span className="text-xs sm:text-sm text-gray-500">{usuarios.length} conectados</span>
      </div>
        ...
        {usuarios.map((usuario, index) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-center space-x-3 p-3 sm:p-4 hover:bg-gray-50 rounded-lg transition-colors duration-200 cursor-pointer"
            >
              <div className="relative">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full ${colorClass} flex items-center justify-center flex-shrink-0`}>
                  <IconComponent className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <div className={`absolute -bottom-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 ${statusColor} rounded-full border-2 border-white`}></div>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-2">
                  ...
                  <span className="text-xs sm:text-sm text-gray-500 whitespace-nowrap sm:ml-2">
                ...
                <div className="flex items-center space-x-1 mt-1 sm:mt-2">
                  <ActivityIcon className="w-3 h-3 sm:w-4 sm:h-4 text-gray-500" />
                  <span className="text-xs sm:text-sm text-gray-500">{activityInfo.text}</span>
                </div>
              </div>
      ...
      {usuarios.length === 0 && (
        <div className="text-center py-6 sm:py-8">
          <FiUser className="w-10 h-10 sm:w-12 sm:h-12 text-gray-400 mx-auto mb-2" />
          <p className="text-sm sm:text-base text-gray-600">No hay usuarios activos</p>
        </div>
      )}
    </div>
  )
